...
{
    public class BusinessDetailForRefundReporter: IBusinessDetailForRefundReporter
    {
        private readonly ApplicationDbContext _context;

        public BusinessDetailForRefundReporter(ApplicationDbContext context)
        {
            _context = context;
        }
        ...
        {
            var billIds = await _context.Bills.Where(x => x.CreatedDate >= fromAt && x.CreatedDate <= toAt 
                                            && (branchId == null || x.BranchId == branchId)).Select(x => x.Id).ToListAsync();

            var itemOuts = await _context.BillDetailRefunds
                .Where(x => billIds.Contains(x.BillId ?? 0))
                .Join(_context.Goods,
                b => b.GoodsId,
                g => g.Id,
                (b, g) => new
            ...
            var itemOutPreviousPeriods = await _context.BillDetailRefunds
                .Where(x => billIds.Contains(x.BillId ?? 0))
                .Join(_context.Goods,
                b => b.GoodsId,
                g => g.Id,
