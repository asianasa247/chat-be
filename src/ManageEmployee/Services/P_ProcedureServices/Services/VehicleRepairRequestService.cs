...
public class VehicleRepairRequestService : IVehicleRepairRequestService
{
    ...
    private async Task<string> GetCodeAsync()
    {
        ...
        var codeNumber = await _context.VehicleRepairRequests.Where(x => x.IsFinished && x.Date.Month == month).OrderByDescending(x => x.Id).Select(x => x.Code).FirstOrDefaultAsync();
