...
namespace ManageEmployee.Controllers.ChatboxAIControllers
{
    ...
    [ApiController]
    public class ChatboxAIQAController : ControllerBase
    {
        private readonly IChatboxAIQAService _service;
        private readonly IMapper _mapper;

        public ChatboxAIQAController(IChatboxAIQAService service, IMapper mapper)
        {
            _service = service;
            _mapper = mapper;
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] ChatboxAIQA model)
        {
            try
            {
        ...
        [HttpPut("{id:int}")]
        public async Task<IActionResult> Update(int id, [FromBody] ChatboxAIQA model)
        {
            try
            {
                if (id <= 0) return BadRequest(new { msg = "Invalid id" });
                model.Id = id; // Ép id từ route

                var result = await _service.Update(model); // <- sửa dùng _service
                if (string.IsNullOrEmpty(result)) return Ok();
